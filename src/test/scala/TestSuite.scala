...
class TestSuite extends FunSuite {
  test("reconstruct comment") {
    val xml"<wrapNode><!--foo--></wrapNode>" = <wrapNode/>
  }
  
  test("reconstruct text") {
    val xml"<wrapNode><![CDATA[foo]]></wrapNode>" = <wrapNode><![CDATA[foo]]></wrapNode>
  }
  ...
  test("reconstruct parameter entity ref"){
    val xml"<foo>%param;</foo>" = <foo>%param;</foo>
  } 

  test("reconstruct proc instr") {
    val xml"<foo><?foo bar?></foo>" = <foo><?foo bar?></foo>
  }
  ...
  test("reconstruct elem with unprefixed attributes") {
    val xml"""<foo a="a" b="b"/>""" = <foo a="a" b="b"/>
  }

  test("reconstruct elem with prefixed attributes") {
    val xml"""<foo a:a="a" b:b="b"/>""" = <foo a:a="a" b:b="b"/>
  }
